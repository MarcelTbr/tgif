...
var senate_data = document.getElementById("senate-data")  //FUTURE: table#senate_data | has to be global?

makeThead()

//3. FETCHING data for the senate.html TABLE ====================\\
...
function makeThead() {
    ...
    return thead
} //FUTURE: makeThead(table) | table#senate-data #157 GLOBAL VAR

// creating tbody
var tbody = document.createElement('tbody')
senate_data.appendChild(tbody)

//Inputs: table#senate_data  & tbody  | Work: 1. Makes new tbbody. 2. Resets it.fills tbody with selected info from the senate_data's JSON | Output: fillRow() #271
function fillTable() {

    tbody = senate_data.children[1] //NOTE: super important line. Defnines the local variable for the nested functions
    ...
    //Input: table#senate_data | Work: makeTbody(senate_data)
    ...
    while (i < members.length) {
        ...
        //Work: puts all the selected variables into the CURRENT ROW of the iteration. This Function MUST BE inside the while loop, because of the index.
        function fillRow() {
        ...
        function classFound(array) {
            ...
            var party = row.className
                //console.log("row.className is: " + party) //FEEDBACK:  row.className
            ...
            if (rowsClassFound == -1) {
                ...
                //console.log("there was a match! nice!") //FEEDBACK: testing classFound(array)
                return true
            }
        } //NOTE: extended Description | gets the non-matching rows out of the HTML. Depending on the array of selected checkboxes given by the function chechboxValues() stored in var getArray. 
...
//                console.log("state_found_cond is: " + state_found_cond)
        ...
        function rowFiltering() {
            ...
            if (outCondition && stateCondition) {
                ...
                //removing the html ot the unqualified row
                row.remove();
            }
